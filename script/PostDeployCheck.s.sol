...
contract PostDeployCheck is Script {
    ...
    function run() external {
        ...
        _check("Timelock has DEFAULT_ADMIN_ROLE", govToken.hasRole(govToken.DEFAULT_ADMIN_ROLE(), timelockAddr));
        _check("Timelock has MINTER_ROLE", govToken.hasRole(govToken.MINTER_ROLE(), timelockAddr));
        _check(
            "Deployer does NOT have DEFAULT_ADMIN_ROLE",
            !govToken.hasRole(govToken.DEFAULT_ADMIN_ROLE(), json.readAddress(".deployer"))
        );
        _check("totalSupply <= MAX_SUPPLY", govToken.totalSupply() <= govToken.MAX_SUPPLY());
        _check("CLOCK_MODE is timestamp", keccak256(bytes(govToken.CLOCK_MODE())) == keccak256(bytes("mode=timestamp")));

        // ── TIMELOCK ──────────────────────────────────────────────────────────
        _section("PredictionTimelock");
        _check("minDelay == 2 days", timelock.getMinDelay() == 2 days);
        _check("REQUIRED_MIN_DELAY == 2 days (constant)", timelock.REQUIRED_MIN_DELAY() == 2 days);
        _check("Governor has PROPOSER_ROLE", timelock.hasRole(timelock.PROPOSER_ROLE(), governorAddr));
        _check("Governor has CANCELLER_ROLE", timelock.hasRole(timelock.CANCELLER_ROLE(), governorAddr));
        _check("address(0) has EXECUTOR_ROLE (open execution)", timelock.hasRole(timelock.EXECUTOR_ROLE(), address(0)));
        _check(
        ...
        _check("votingDelay == 1 day", governor.votingDelay() == 1 days);
        _check("votingPeriod == 1 week", governor.votingPeriod() == 1 weeks);
        _check("quorumNumerator == 4", governor.quorumNumerator() == 4);
        _check("CLOCK_MODE is timestamp", keccak256(bytes(governor.CLOCK_MODE())) == keccak256(bytes("mode=timestamp")));
        _check("Timelock is the governor's timelock", address(governor.timelock()) == timelockAddr);

        // ── CONDITIONAL TOKENS ────────────────────────────────────────────────
        _section("ConditionalTokens");
        _check("Timelock has DEFAULT_ADMIN_ROLE", ct.hasRole(ct.DEFAULT_ADMIN_ROLE(), timelockAddr));
        _check("Factory has FACTORY_ROLE", ct.hasRole(ct.FACTORY_ROLE(), factoryAddr));
        _check("Market1 is authorized", ct.isAuthorizedMarket(market1Addr));

        // ── FEE VAULT ─────────────────────────────────────────────────────────
        _section("FeeVault");
        _check("Timelock has DEFAULT_ADMIN_ROLE", vault.hasRole(vault.DEFAULT_ADMIN_ROLE(), timelockAddr));
        _check("Factory has SOURCE_ROLE", vault.hasRole(vault.SOURCE_ROLE(), factoryAddr));

        // ── MARKET FACTORY ────────────────────────────────────────────────────
        _section("MarketFactory");
        _check("Implementation matches deployed impl", MarketFactory(factoryAddr).implementation() == implAddr);
        _check(
            "Timelock has CREATOR_ROLE",
            MarketFactory(factoryAddr).hasRole(MarketFactory(factoryAddr).CREATOR_ROLE(), timelockAddr)
        );
        _check("allMarketsLength == 1", MarketFactory(factoryAddr).allMarketsLength() == 1);

        // ── MARKET #1 ─────────────────────────────────────────────────────────
        _section("PredictionMarket #1");
        _check("Timelock has DEFAULT_ADMIN_ROLE", market.hasRole(market.DEFAULT_ADMIN_ROLE(), timelockAddr));
        _check("Timelock has UPGRADER_ROLE", market.hasRole(market.UPGRADER_ROLE(), timelockAddr));
        _check(
            "Deployer does NOT have DEFAULT_ADMIN_ROLE on market",
            !market.hasRole(market.DEFAULT_ADMIN_ROLE(), json.readAddress(".deployer"))
        );
        _check("AMM NOT frozen at launch", !market.ammFrozen());
        _check("Market state == Active (0)", uint8(market.state()) == 0);
        _check("closeTime > now", market.closeTime() > uint64(block.timestamp));
