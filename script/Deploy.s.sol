...
contract Deploy is Script {
    ...
    function run() external {
        ...
        ct.grantRole(ct.FACTORY_ROLE(), address(factory));
        vault.grantRole(vault.SOURCE_ROLE(), address(factory));

        // Hand off admin to Timelock
        ct.grantRole(ct.DEFAULT_ADMIN_ROLE(), address(timelock));
        vault.grantRole(vault.DEFAULT_ADMIN_ROLE(), address(timelock));
        ct.revokeRole(ct.DEFAULT_ADMIN_ROLE(), deployer);
        vault.revokeRole(vault.SOURCE_ROLE(), deployer);
        vault.revokeRole(vault.DEFAULT_ADMIN_ROLE(), deployer);

        // ── 9. Oracle resolver ────────────────────────────────────────────────
        resolver = new ChainlinkResolver(
            AggregatorV3Interface(CHAINLINK_ETH_USD), STALENESS_TOLERANCE, "ETH/USD Arbitrum Sepolia"
        );
        ...
        (market1,) = factory.createMarket(
            MarketFactory.CreateParams({
                ...
                thresholdPrice: 3000e8, // $3 000 (8 decimals)
